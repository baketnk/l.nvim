...
function M.setup(_opts)
	...
	M.make_plugKey("SetAllFiles", "n", "F", lcmd.select_all_files_for_prompt, { desc = "Select hidden/ignored files" })
	...
	M.make_plugKey(
		...
		"n",
		"R",
		lcmd.smart_replace_with_codeblock,
	...
	M.make_plugKey(
		...
		"n",
		"sn",
		require("lnvim.memex").global_search,
		{ desc = "Search memex content" }
	)
	...
	M.make_plugKey(
		...
		function()
			...
			local message = string.format(
				"Reasoning mode %s%s",
				status,
				state.use_reasoning and has_override and " (using custom prompt for " .. model.model_id .. ")" or ""
			)
